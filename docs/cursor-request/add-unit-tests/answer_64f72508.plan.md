---
name: Добавление юнит-тестирования
overview: Добавление системы юнит-тестирования с использованием Jest, конфигурационного файла для опций тестов, теста доступности DNS-сервера и нагрузочного теста.
todos:
  - id: install-deps
    content: Установить Jest и необходимые зависимости (@jest/globals, @types/jest) в package.json
    status: pending
  - id: jest-config
    content: Создать jest.config.mjs с настройками для ESM и TypeScript
    status: pending
    dependencies:
      - install-deps
  - id: test-config-files
    content: Создать config/test.config.ini и config/test.config.ini.dist с опциями тестов
    status: pending
  - id: test-options-loader
    content: Создать src/Test/TestOptionsLoader.ts для загрузки опций тестов из INI файла
    status: pending
    dependencies:
      - test-config-files
  - id: availability-test
    content: Создать tests/App.test.ts с тестом доступности DNS-сервера
    status: pending
    dependencies:
      - test-options-loader
      - jest-config
  - id: load-test
    content: Создать tests/LoadTest.test.ts с нагрузочным тестом
    status: pending
    dependencies:
      - test-options-loader
      - jest-config
  - id: update-package-scripts
    content: "Обновить package.json: добавить скрипты test, test:watch, test:coverage"
    status: pending
    dependencies:
      - jest-config
---

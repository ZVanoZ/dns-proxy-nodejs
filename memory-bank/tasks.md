...
- **Где определять необходимость IPv6:** при старте приложения по конфигурации: наличие секции/опций для IPv6 (например, `bindHost6` или `port6`) включает создание сокета `udp6`.
...
| Шаг | Действие |
|-----|----------|
| 2.1 | Конфигурация `config/app.ini`: добавить опции для IPv6 (например, `bindHost6`, при одном порте — общий `port`; при двух — `port6`). Формат простой и понятный. |
| 2.2 | Опции приложения: расширить интерфейс/класс опций (bindHost6?, port6? или единый port для обоих). Загрузка из INI. |
...
План утверждён. Дальше: **CREATIVE** (при необходимости для деталей архитектуры) → **VAN QA** → **BUILD**.

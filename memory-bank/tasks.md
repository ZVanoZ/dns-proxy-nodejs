...
**Описание:**
При запуске PROD режима через Docker возникает ошибка `EACCES` при попытке биндинга на порт 53. Необходимо исправить конфигурацию для корректной работы PROD режима.

**Проблема:**
- Ошибка: `bind EACCES 0.0.0.0:53`
- Причина: Приложение запускается от пользователя `appuser` в контейнере, даже с `cap_add: NET_BIND_SERVICE` не может биндить привилегированный порт 53
- Текущая конфигурация: `network_mode: host` + `cap_add: NET_BIND_SERVICE`

**Требования:**
1. PROD должен биндить порт 53 на хост-машине (соответствие первоначальному плану)
2. Рассмотреть вариант с запуском на порту 5053 в контейнере с маппингом на порт 53 на хосте
3. Проанализировать техническую возможность и соответствие первоначальному плану

**Статус:**
- [x] Анализ проблемы
- [x] Планирование решения
- [x] Реализация изменений
- [x] Тестирование
- [x] Задача выполнена ✅

**Прогресс реализации:**
- [x] Этап 1: Обновление docker-compose.yml для PROD
- [x] Этап 2: Обновление app.ini.dist для PROD (уже был настроен на 5053)
- [x] Этап 3: Обновление Dockerfile
- [x] Этап 4: Обновление документации
- [x] Этап 5: Тестирование конфигурации

**Результаты тестирования:**

✅ **Тестирование конфигурации:**
- docker-compose.yml обновлен: убран `network_mode: host`, добавлен `ports: "53:5053/udp"`, убран `cap_add: NET_BIND_SERVICE`
- Конфигурация валидна (docker compose config проходит без ошибок)
- Порт правильно настроен: `target: 5053, published: "53"`

✅ **Обновление файлов:**
- `env/prod/Dockerfile`: EXPOSE обновлен на `5053/udp`
- `env/prod/config/app.ini.dist`: уже настроен на порт 5053
- `docs/DOCKER-OPTIONS.md`: все примеры обновлены с новыми настройками

**Изменения:**
- PROD режим теперь использует bridge network вместо host network
...
**Важно:** Если пользователь уже создал `env/prod/config/app.ini`, необходимо обновить в нем порт на 5053:
```ini
[app]
host=0.0.0.0
port=5053  # Изменить с 53 на 5053
```

Шаблон `env/prod/config/app.ini.dist` уже настроен на порт 5053.
